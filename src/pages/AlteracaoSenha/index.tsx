...
function AlteracaoSenha() {
    ...
    const errors = {
        senhaAtual: obrigatorio(senhaAtual) || max(senhaAtual, 120) || senhaAtualInvalida,
        novaSenha:
            obrigatorio(novaSenha)
                || max(novaSenha, 120)
                || novaSenha != novaSenhaConfirm ? 'Senhas diferentes' : null,
        novaSenhaConfirm:
            obrigatorio(novaSenhaConfirm)
                || max(novaSenhaConfirm, 120)
                || novaSenhaConfirm != novaSenha ? 'Senhas diferentes' : null,
    };

    const formPreenchido = senhaAtual != '' && novaSenha != '' && novaSenhaConfirm != '';
    const formValido = !errors.senhaAtual && !errors.novaSenha && !errors.novaSenhaConfirm;
    ...
    function handleSubmit() {
        ...
        setLoading(true);
        api.put(
            `usuarios/${usuarioLogado?.id}/senha`,
            {
            ...
            if (titulo == 'Senha atual informada não coincide com a senha do usuário.') {
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.label}>Confirmação da nova senha:
               {formSubmetido
                    && errors.novaSenhaConfirm
                    && <Text style={styles.textoValidacao}>{`\b${errors.novaSenhaConfirm}`}</Text>}
            </Text>
            <TextInput
                style={
                    [
